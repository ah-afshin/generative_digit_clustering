...
def plot_tsne(embeddings: t.Tensor, labels: t.Tensor, perplexity: int = 30, n_samples: int = 2000) -> None:
    ...
    embeddings_np = embeddings_np[:n_samples]
    labels_np = labels_np[:n_samples]

    tsne = TSNE(n_components=2, perplexity=perplexity, init='pca', random_state=42)
    reduced = tsne.fit_transform(embeddings_np)  # [n_samples, 2]

    plt.figure(figsize=(8, 6))
    scatter = plt.scatter(reduced[:, 0], reduced[:, 1], c=labels_np, cmap='tab10', s=15)
    plt.colorbar(scatter, ticks=range(10), label='Digit')
    plt.title(f't-SNE Visualization of Encodings (perplexity={perplexity})')
    plt.tight_layout()
    plt.show()
...
if __name__=="__main__":
    ...
    plot_tsne(encodec_vec, true_labels)
